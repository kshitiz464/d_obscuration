# # models/drone_swarm_env.py
# """
# Custom Gym environment for training RL agent on drone swarm coordination.
# Priorities:
# 1. Avoid collisions
# 2. Reach and stabilize at target destinations
# 3. Minimize time to target
# """
# import gym
# import numpy as np
...
#         # Observation: positions and velocities of all drones
...
#         # Action: force vector for each drone (x, y, z)
...
#         self.targets = FormationPlanner.staggered_pattern(self.num_drones, self.horizontal_spacing, self.vertical_spacing, self.altitude)
...
#             total_dist += dist
#             if dist < 1.0:
#                 reward += 1  # small reward for proximity
#             if np.linalg.norm(new_velocities[i]) < 0.5 and dist < 1.5:
#                 reward += 2  # bonus for stability near target
...
class DroneSwarmEnv(gym.Env):
    ...
    def __init__(self,
                 ...
                 horizontal_spacing: float = 20.0,
                 vertical_spacing: float = 5.0,
                 wind_ax: float = 0.002,   # 💨 reduced wind
                 wind_ay: float = 0.002,
                 seed: int = None):
    ...
    def step(self, action: np.ndarray):
        ...
        for i in range(self.num_drones):
            ...
            # Wind and drag effects
            ax += self.wind_ax
            ay += self.wind_ay

            drag = 0.005  # 🧲 reduced drag
            ax -= drag * vx
            ay -= drag * vy
            az -= drag * vz
        ...
        reward -= 0.1  # baseline penalty

        # Optional future: adapt reward over time (based on average performance)
        # TODO: Adaptive reward strategy placeholder
